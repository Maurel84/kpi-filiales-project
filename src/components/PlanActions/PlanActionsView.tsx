...
type PlanActionInsert = Omit<PlanAction, 'id'> & { filiale_id?: string | null };
...
export function PlanActionsView() {
  ...
  const [formData, setFormData] = useState<PlanActionInsert>({
    action: '',
  ...
  const loadActions = useCallback(async () => {
    ...
    setLoading(false);
  }, [filterPriorite, filterStatut, profile]);
  ...
  const submitAction = async () => {
    if (!profile) {
      setSubmitError('Aucun profil connecté.');
      return;
    }
    if (!formData.action) {
      setSubmitError('Le libellé de l’action est requis.');
      return;
    }
    ...
    const payload: PlanActionInsert = {
      ...formData,
      filiale_id: profile.filiale_id || null,
      responsable_nom: formData.responsable_nom || `${profile.prenom ?? ''} ${profile.nom ?? ''}`.trim(),
    };
    ...
    setFormData({
      ...
      date_action: new Date().toISOString().slice(0, 10),
      niveau_priorite: 'Moyenne',
      responsable_nom: profile?.prenom ? `${profile.prenom} ${profile.nom}` : '',
      date_fin_prevue: '',
      statut: 'En_cours',
      commentaires: '',
      filiale_id: profile?.filiale_id || null,
