...
## Power BI & Edge Functions (Supabase)

- Table Power BI :
  - `supabase/sql/powerbi_configs.sql` : crée `powerbi_configs` (embed_url, dataset_id, workspace_id, id = user id).
- Policies RLS :
  - `supabase/rls_policies.sql` : exemples d’insert pour `admin_siege` / `manager_filiale` (ventes, commandes, visites, opportunités, plan_actions) + requête de vérification des profils.
- Edge Functions à déployer sur Supabase :
  - `supabase/functions/get-embed-token` : génère un token d’embed Power BI via un service principal Azure AD (vars env : AZURE_TENANT_ID, PBI_CLIENT_ID, PBI_CLIENT_SECRET, PBI_WORKSPACE_ID, PBI_REPORT_ID, PBI_DATASET_ID, POWERBI_SCOPE).
  - `supabase/functions/import-forecast` : importe `FORECAST.csv` (bucket par défaut `imports`) dans `modeles_produits`, `historique_ventes_modeles`, `previsions_ventes_modeles` (année 2026). Vars env : SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, FORECAST_BUCKET, FORECAST_PATH.
- Frontend Power BI :
  - Menu “Power BI” accessible aux rôles `admin_siege` / `manager_filiale`.
  - Écran `src/components/PowerBI/PowerBIView.tsx` pour saisir/sauver workspace, dataset, embed URL (via `powerbi_configs`). Le token d’embed doit être obtenu via la fonction Edge (non exposée au front).
